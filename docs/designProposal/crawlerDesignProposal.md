...
Responsible for downloading the HTML content of a webpage.

The Fetcher automatically determines the appropriate fetching strategy based on the webpage type.

Two fetching mechanisms are supported:

#### Static / Server-Side Rendered Websites

For websites whose HTML is available directly from the server, the Fetcher uses **libcurl**.
...
This method is fast and lightweight because the HTML is immediately available in the server response.

---

#### Client-Side Rendered (Dynamic) Websites

Some modern websites (React, Angular, Vue, etc.) initially return very little HTML and construct the page using JavaScript.

For such websites, the Fetcher uses the **Chrome DevTools Protocol (CDP)**.
...
Execute JavaScript
 ↓
Wait until DOM finishes rendering
 ↓
...
By using CDP, the crawler receives the fully rendered DOM exactly as a browser would see it, allowing hyperlinks generated by JavaScript to be extracted.

The Fetcher hides these implementation details from the Crawler. Regardless of the underlying strategy, it simply returns the final HTML document.
...
- Fetching strategy is transparent to the crawler.
...
- Dynamic website support depends on the availability of a Chromium browser supporting the Chrome DevTools Protocol.